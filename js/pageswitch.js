(function($){
	...
	$('#next').on('click',SP.moveSectionDown);
	...
	function navAClick() {
		var navA = $('#wrap > a');
		var length = navA.length;
		for(var i=0; i < length; i++) {
			navA.eq(i).index = i;
			navA.eq(i).on('click', function() {
	...
	function togetherEvent() {
		var navA = $('#wrap > a');
		var navLi = $('.nav li');
		navA.eq(iIndex).addClass('current').siblings().removeClass('current');
		navLi.eq(iIndex).addClass('current').siblings().removeClass('current');
	}
